...
public class GetOut : Intention {
	...
	private void SelectCheckPoint(Agent agent){
        ...
        if(cpInSight.Contains(agent.OnCheckpoint))
        {
            cpInSight.Remove(agent.OnCheckpoint);
        }
		...
		agent.OnCheckpoint = null;
	}

	private Vector3 FollowIndication(Vector3 position, GameObject indication){
		return (Vector3.Distance(position, indication.transform.position) < 3 ? // if i am close enough
